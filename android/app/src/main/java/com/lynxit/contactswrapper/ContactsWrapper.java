...
public class ContactsWrapper extends ReactContextBaseJavaModule implements ActivityEventListener {
                ...
                switch(requestCode) {
                    case(CONTACT_REQUEST):
                        try {
                            ...
                            if (cursor.moveToFirst()) {
                                do {
                                    ...
                                    if(returnKeys.containsKey(mime)) {
                                        if("phone".equals(returnKeys.get(mime))){
                                            phoneList.add(cursor.getString(dataIdx));
                                        }
                            ...
                            if(foundData) {
                                if(phoneList.size() > 1){
                                    showListDialog(contactData,phoneList);
                                    return;
                                }
    ...
    private void showListDialog(final WritableMap contactData,List<String> itemList) {
        ...
        listDialog.setItems(items, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                contactData.putString("phone",items[which]);
                mContactsPromise.resolve(contactData);
            }
        });
        listDialog.show();
